...
export class WeatherService {
  ...
  displayWeather: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(false);
  ...
  submitLocation(): void {
    this.http
      .get<any>(`${this.geocodeUrl}?q=${this.city}+${this.state}&api_key=${this.geocodeKey}`)
      .subscribe(data => {
        this.getWeatherData(data.results[0].location.lat, data.results[0].location.lng);
      });
  }
